...
export const Watch = (initialValue) => {
  ...
  const set = (newValueOrUpdater) => {
    const newValue = typeof newValueOrUpdater === 'function'
      ? newValueOrUpdater(value) // functional update
      : newValueOrUpdater;
 ...
 * Specifies the spacing direction.
 * Use `'width'` for horizontal spacing or `'height'` for vertical spacing.
 *
 * @param {number} value
 * The size of the spacer in pixels.
 *
 * @returns {HTMLDivElement}
 * A div element styled with the specified width or height.
 *
 * @throws {Error}
 * Throws an error if `value` is not a number or if `axis` is not `'width'` or `'height'`.
 *
 * @example
 * // Vertical space
 * container.appendChild(SizedBox('height', 16));
 *
 * @example
 * // Horizontal space
 * row.appendChild(SizedBox('width', 24));
 */
export function SizedBox(axis, value) {
...
export class Validator {
  ...
  isPassword(options = {}) {
    ...
    if (requireLetter && !/[a-zA-Z]/.test(this.value)) {
      throw new Error('Password must contain at least one letter');
    }

    if (requireNumber && !/[0-9]/.test(this.value)) {
      throw new Error('Password must contain at least one number');
    }

    if (requireSpecial && !/[!@#$%^&*(),.?":{}|<>]/.test(this.value)) {
  ...
  maxLength(length) {
    if (
      ...
      throw new Error(`Length must be at most ${length}`);
    }
    return this;
  }
